  ...
  var FSTK = function(){
    ...
    */
    F['dirPart'] = function(path, options, callback){
      ...
      , function(cb){
          return Async.each(globals.directories, function(d, _cb){
    ...
    F['checksum'] = function(value, options){
      options || (options = {})
    ...
    F['fileChecksum'] = function(filename, options, callback){
    ...
    F['checksumFork'] = function(path, callback){
      return Child_Process.exec('sudo ionice -c 1 sha1sum "' + path + '"', function(err, stdout, stderr){
        if (!err && stderr) err = new Error(stderr);
        if (err) return callback(err);
        var cs = stdout.split(' ').shift();
        if (!cs) return callback(new Error('Checksum not found'));
        return callback(null, cs);
      });
    };

    F['ulimitQueue'] = Async.priorityQueue(function(task, cb){
      return task(cb);
    }, process.env.ULIMIT_CONCURRENCY || 1);

    F['mtimeQueue'] = Async.priorityQueue(function(task, cb){
      return task(cb);
    }, process.env.MTIME_CONCURRENCY || 1);

    F['readdirQueue'] = Async.priorityQueue(function(task, cb){
      return task(cb);
    }, process.env.MTIME_CONCURRENCY || 5);

    F['fastWalk'] = function(path, store, callback){
      ...
      if (process.env.FSTK_VERBOSE) console.log(path);
      ...
      , function(stat, cb){
          if (stat.isSymbolicLink()) return cb('LINK', stat);
          if (stat.isFile()) return cb('FILE', stat);
          if (!stat.isDirectory()) return cb('OTHER', stat);
          gstat = stat;

          return self.mtimeQueue.push(function(_cb){
            return FS.readdir(path, function(){
              cb.apply(this, arguments);
              return _cb();
            });
          }, -1 * stat.mtime.getTime());
        }
      , function(files, cb){
          store.dirs[path] = {'stat': true || gstat, 'size': files.length};

          var mtime = -1 * gstat.mtime.getTime();
          /*_.each(files, function(f){
            return self.mtimeQueue.push(function(_cb){
              return self.fastWalk(Path.join(path, f), store, function(){
                return _cb();
              });
            }, mtime);
          });
          ...
          /*return Async.each(files, function(f, _cb){
            return self.readdirQueue.push(function(xb){
              _cb(); 
              return self.fastWalk(Path.join(path, f), store, function(){
                xb();
              });
          ...
          return Async.eachLimit(files, 6, function(f, _cb){
            return self.fastWalk(Path.join(path, f), store, function(){
              return _cb(); 
            });
          }, cb);
        ...
        if (err === 'LINK'){
          store.files[path] = {'stat': true || stat, 'link': true};
        } else if (err === 'FILE'){
          if (true || stat.size === 0){
            store.files[path] = {'stat': true || stat, 'checksum': stat.size};
          } else {
            self.ulimitQueue.push(function(cb){
              return self.fileChecksum(path, function(err, cs){
                if (err){
                  store.files[path] = {'stat': true || stat, 'error': err};
                } else {
                  //if (process.env.FSTK_VERBOSE) console.log({'path': path, 'stat': stat, 'checksum': cs});
                  store.files[path] = {'stat': true || stat, 'checksum': cs};
                }
                return cb();
              });
            }, -1 * stat.mtime.getTime());
          ...
          store.files[path] = {'nonfile': true, 'stat': true || stat};
        } else if (err){
          store.files[path] = {'error': err};
        }
