  ...
  var FSTK = function(){
    ...
    */
    ...
    F['checksumFork'] = function(path, callback){
      return Child_Process.exec('sha1sum "' + path + '"', function(err, stdout, stderr){
    ...
    F['ulimitQueue'] = Async.priorityQueue(function(task, cb){
      return task(cb);
    }, process.env.ULIMIT_CONCURRENCY || 800);

    F['mtimeQueue'] = Async.priorityQueue(function(task, cb){
      return task(cb);
    }, process.env.MTIME_CONCURRENCY || 200);
    ...
    F['fastWalk'] = function(path, store, callback){
      ...
      , function(files, cb){
        ...
        if (err === 'LINK'){
          store.symlinks.push({'path': path, 'stat': stat});
        } else if (err === 'FILE'){
          store.files.push({'path': path, 'stat': stat});

          if (stat.size === 0){
            store.checksums.push({'path': path, 'stat': stat, 'checksum': stat.size});
          } else {
            self.ulimitQueue.push(function(cb){
              //return self.checksumFork(path, function(err, cs){
              return self.fileChecksum(path, function(err, cs){
                if (err){
                  store.errors.push({'path': path, 'stat': stat, 'error': err.message});
                } else {
                  //if (process.env.FSTK_VERBOSE) console.log({'path': path, 'stat': stat, 'checksum': cs});
                  store.checksums.push({'path': path, 'stat': stat, 'checksum': cs});
                }
                return cb();
              });
            }, -1 * stat.mtime.getTime());
          ...
          store.nonfiles.push({'path': path, 'stat': stat});
        } else if (err){
          store.errors.push({'path': path, 'error': err});
        }
